...
def receive_pose(buffersize, connect):
	send_str = 'cart_pos\r'
	connect.sendall(send_str.encode())
	raw_data = connect.recv(buffersize)
...
def applyHandEye(point, R_cam2gripper, t_cam2gripper, cameraMatrix, connect):
	gripper_trans = np.zeros((3,1))
	gripper_orient = np.zeros((3,1))
	gripper_pose = receive_pose(1024, connect)
	#print(gripper_pose)
	...
	point[0] = point[0]*(gripper_trans[2]+t_cam2gripper[2])
	point[1] = point[1]*(gripper_trans[2]+t_cam2gripper[2])
	point[2] = point[2]*(gripper_trans[2]+t_cam2gripper[2])
	#print(point)
